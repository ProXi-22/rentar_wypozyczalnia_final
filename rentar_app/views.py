...
from .forms import (RegisterForm, LoginForm, DodajSamochodForm, EdytujSamochodForm,
                    RezerwacjaForm, FiltrSamochodowForm, DodajPracownikaForm,
                    EdytujPracownikaForm, RaportForm)


def sprawdz_role(user, wymagana_rola):
    try:
        uzytkownik = Uzytkownik.objects.get(user=user)
        return uzytkownik.rola == wymagana_rola
...
def sprawdz_role_pracownik_lub_admin(user):
    try:
        uzytkownik = Uzytkownik.objects.get(user=user)
        return uzytkownik.rola in ['pracownik', 'admin']
    except Uzytkownik.DoesNotExist:
        return False
...
def index(request):
    ...
    context = {
        ...
        'liczba_rezerwacji': liczba_rezerwacji,
    }
...
def cars_list(request):
    ...
    if form.is_valid():
        if form.cleaned_data.get('nazwa'):
            samochody = samochody.filter(nazwa__icontains=form.cleaned_data['nazwa'])

        if form.cleaned_data.get('cena_od'):
            samochody = samochody.filter(cena_za_dobe__gte=form.cleaned_data['cena_od'])

        if form.cleaned_data.get('cena_do'):
            samochody = samochody.filter(cena_za_dobe__lte=form.cleaned_data['cena_do'])

        if form.cleaned_data.get('moc_od'):
            samochody = samochody.filter(moc__gte=form.cleaned_data['moc_od'])

        if form.cleaned_data.get('moc_do'):
            samochody = samochody.filter(moc__lte=form.cleaned_data['moc_do'])

        if form.cleaned_data.get('sortowanie'):
            samochody = samochody.order_by(form.cleaned_data['sortowanie'])
...
def potwierdz_rezerwacje(request, rezerwacja_id):
    ...
    if rezerwacja.status == 'oczekujacy':
        ...
        messages.error(request, 'Rezerwacja ma już inny status.')
...
def edytuj_samochod(request, samochod_id):
    ...
    if request.method == 'POST':
        ...
        form = EdytujSamochodForm(instance=samochod)

    context = {
        'form': form,
        'samochod': samochod,
        'title': f'Edytuj {samochod.nazwa}',
    }
    return render(request, 'edycja_samochodu.html', context)
...
def edytuj_pracownika(request, uzytkownik_id):
    ...
    if request.method == 'POST':
        ...
        form = EdytujPracownikaForm(instance=user)

    context = {
        'form': form,
        'uzytkownik_profil': uzytkownik_profil,
        'title': f'Edytuj {user.username}',
    }
    return render(request, 'edycja_uzytkownika.html', context)
    ...
    if form.is_valid():
        ...
        if okres == 'dzien':
            ...
            if now.month == 12:
                ...
                data_do = now.replace(month=now.month + 1, day=1)
        elif okres == 'rok':
        ...
        rezerwacje = Rezerwacja.objects.filter(
            data_utworzenia__gte=data_od_dt,
            data_utworzenia__lt=data_do_dt,
            status__in=['potwierdzony', 'zakonczony']
        )
        ...
        if typ_raportu == 'zyski':
            ...
            raport_data = {
                'typ': 'Zyski',
                'wartosc': zyski,
                'okres': f"{data_od} do {data_do}",
                'liczba_rezerwacji': rezerwacje.count(),
            }

        elif typ_raportu == 'popularne':
            popularne = rezerwacje.values('samochod__nazwa').annotate(
                liczba=Count('id'),
                przychod=Sum('kwota_razem')
            ).order_by('-liczba')[:10]
            raport_data = {
                'typ': 'Najpopularniejsze samochody',
                'dane': list(popularne),
            }

        elif typ_raportu == 'pracownicy':
            pracownicy = rezerwacje.values('pracownik_potwierdzajacy__username',
                                           'pracownik_potwierdzajacy__first_name').annotate(
                liczba_rezerwacji=Count('id'),
                przychod=Sum('kwota_razem')
            ).order_by('-liczba_rezerwacji')
            raport_data = {
                'typ': 'Raport pracowników',
                'dane': list(pracownicy),
            }
